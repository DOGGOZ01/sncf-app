...
class MainActivity : AppCompatActivity() {
    private val api: SNCFapi = SNCFapi(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        input.setOnItemClickListener { parent, _, position, _ ->
            ...
            api.fetchDeparturesFromStopArea(selectedStation.CODE_UIC, callback = object : Callback {
                ...
                override fun onResponse(call: Call, response: Response) {
                    this@MainActivity.runOnUiThread {
                        ...
                        val trains = api.parseDepartures(response.body as ResponseBody, selectedStation)

                        trainsViewAdapter!!.addAll(trains)

                        trains.forEach { train ->
                            api.fetchVehicleJourney(train.num_journey, callback = object : Callback {
                                override fun onFailure(call: Call, e: IOException)
                                {
                                    e.printStackTrace()
                                }
                                ...
                                override fun onResponse(call: Call, response: Response)
                                {
                                    api.parseVehicleJourneys(train, response.body!!)
                                }
                            })
                        }
