...
class SNCFapi(
    ...
    fun fetchDeparturesFromStopArea(codeUIC: Int, callback: Callback) {
    ...
    fun fetchVehicleJourney(vehicleJourneyId: String, callback: Callback) {
    ...
    fun parseDepartures(json: ResponseBody, station: Station): List<Train> {
        ...
        try {
            ...
            for (i in 0 until departures.length()) {
                ...
                val stopFrom = Stop(heure, minute, heure, minute).apply { setStation(station) }
    ...
    fun parseVehicleJourneys(train: Train, json: ResponseBody): Train {
        ...
        for (i in 0 until stopTimes.length()) {
            ...
            val arrivalH = stopTime.getString("arrival_time").chunked(2)[0]
            val arrivalM = stopTime.getString("arrival_time").chunked(2)[1]

            val departureH = stopTime.getString("departure_time").chunked(2)[0]
            val departureM = stopTime.getString("departure_time").chunked(2)[1]
            ...
            val stop = Stop(arrivalH, arrivalM, departureH, departureM)
